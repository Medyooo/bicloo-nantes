...
describe('GET /stations', function() {
    ...
    it('retourne 200 avec la liste des stations', async function() {
        nock(API_HOST)
            .get(API_PATH)
            .query({ limit: 100 })
        ...
        expect(response.result[0].name).to.equal('Commerce');
    });
    ...
    it('retourne 200 avec le détail d\'une station', async function() {
        nock(API_HOST)
            .get(API_PATH)
            .query({ limit: 100 })
            .reply(200, { results: [FAKE_STATION_OPEN] });
        ...
        expect(response.result.name).to.equal('Commerce');
    });
